...
open class ODataOnboardingStep2: OnboardingStep {
    ...
    private func configureOData(using context: OnboardingContext, completionHandler: @escaping (OnboardingResult) -> Void) {
        ...
        odataControllers[destinations["com.sap.edm.sampleservice.v2"] as! String] = Comsapedmsampleservicev2OfflineODataController()
        ...
        for (destinationId, odataController) in odataControllers {
            ...
            do {
                ...
                odataController.openOfflineStore(synchronize: true) { error in
                    if let error = error {
